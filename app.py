...
def load_static_data():
    customers = pd.DataFrame({
        ...
        'delivery_window_start_minutes': [540, 480, 600, 540, 660, 480, 720, 540, 600, 660],
        'delivery_window_end_minutes': [1020, 960, 1080, 1020, 1140, 960, 1200, 1020, 1080, 1140]
    })
    return customers
...
def run_dispatch_optimization(scenario='normal'):
    ...
    # Merge trailers with customer data to get delivery windows
    trailers_with_customers = pd.merge(
        trailers,
        customers_df[['name', 'delivery_window_start_minutes', 'delivery_window_end_minutes']],
        left_on='destination', right_on='name', how='left'
    )
    # Fill NaN delivery windows for 'Mother Station' or unknown destinations
    trailers_with_customers['delivery_window_start_minutes'] = trailers_with_customers['delivery_window_start_minutes'].fillna(0)
    trailers_with_customers['delivery_window_end_minutes'] = trailers_with_customers['delivery_window_end_minutes'].fillna(1440) # 24 hours

    # Initialize new metrics
    on_time_delivery_rate = 0.0
    simulated_empty_miles_reduction = 0.0
    simulated_carbon_reduction_tons = 0.0

    if scenario == 'breakdown':
        trailers_with_customers.loc[trailers_with_customers['trailer_id'] == 'T-001', 'eta_minutes'] += 60
        trailers_with_customers.loc[trailers_with_customers['trailer_id'] == 'T-001', 'status'] = 'Delayed'
        # Simulate worse metrics for breakdown scenario
        simulated_empty_miles_reduction = 0.05 # 5% reduction
        simulated_carbon_reduction_tons = 10.0 # 10 tons reduction
    else:
        # Simulate better metrics for normal scenario
        simulated_empty_miles_reduction = 0.15 # 15% reduction
        simulated_carbon_reduction_tons = 30.0 # 30 tons reduction

    # Calculate on-time delivery rate
    on_time_deliveries = 0
    total_deliveries_scheduled = 0
    for _, row in trailers_with_customers.iterrows():
        if row['destination'] != 'Mother Station': # Only consider actual deliveries
            total_deliveries_scheduled += 1
            # Simulate actual arrival time slightly varied from ETA
            actual_arrival_minutes = row['eta_minutes'] + random.randint(-10, 10)

            if (actual_arrival_minutes >= row['delivery_window_start_minutes']) and                (actual_arrival_minutes <= row['delivery_window_end_minutes']):
                on_time_deliveries += 1

    on_time_delivery_rate = (on_time_deliveries / total_deliveries_scheduled) * 100 if total_deliveries_scheduled > 0 else 100

    total_trailers = len(trailers_with_customers)
    active = len(trailers_with_customers[trailers_with_customers['status'].isin(['Moving', 'Unloading'])])
    utilization = (active / total_trailers) * 100 if total_trailers > 0 else 0
    total_load = trailers_with_customers['load_mmbtu'].sum()
    ...
    # Return the original trailers DataFrame (without customer merge columns) along with metrics
    return trailers, utilization, total_load, dispatched_orders, on_time_delivery_rate, simulated_empty_miles_reduction, simulated_carbon_reduction_tons
...
def calculate_settlement(start_date, end_date, customers_data=None):
    ...
    for _, row in merged.iterrows():
        ...
        results.append({
            ...
            'actual_cng_emissions': round(actual_cng_emissions, 2),
...
def forecast_demand(customer_id, days=7):
    ...
    daily_avg = customer_deliveries.groupby('date')['delivered_mmbtu'].sum().mean()
...
# ----------------------------
    ...
    if st.button("🔄 Run Optimization"):
        if scenario == "🚨 Breakdown Contingency (T-001)":
            trailers, utilization, total_load, orders, on_time_delivery_rate, simulated_empty_miles_reduction, simulated_carbon_reduction_tons = run_dispatch_optimization(scenario='breakdown')
            st.warning("⚠️ Contingency Plan Activated: T-001 re-routed. ETA extended by 60 mins.")
        else:
            trailers, utilization, total_load, orders, on_time_delivery_rate, simulated_empty_miles_reduction, simulated_carbon_reduction_tons = run_dispatch_optimization(scenario='normal')
            st.success("✅ Optimization Complete: All schedules optimized.")

        st.session_state.trailers = trailers

        # Convert total_load from MMBTU to MScf for display
        total_load_mscf = total_load / MScf_TO_MMBTU_FACTOR
        ...
        col3.metric("📦 Total Dispatched (MScf)", f"{total_load_mscf:,.0f}")
        col4.metric("🚚 On-Time Delivery Rate", f"{on_time_delivery_rate:.1f}%")
        col5.metric("📉 Empty Miles Reduction", f"{simulated_empty_miles_reduction:.1%}")
        col6.metric("🌳 Carbon Reduction", f"{simulated_carbon_reduction_tons:.1f} tons")
        ...
        # Convert load_mmbtu to load_mscf for display in the dataframe
        trailers_display = trailers.copy()
        trailers_display['load_mscf'] = trailers_display['load_mmbtu'] / MScf_TO_MMBTU_FACTOR
        st.dataframe(trailers_display[['trailer_id', 'status', 'destination', 'eta_minutes', 'load_mscf', 'pressure_psi']].rename(columns={'load_mscf': 'load_mscf'}), use_container_width=True)
...
# ----------------------------
    ...
    if filtered_settlement.empty:
        st.warning("No delivery data found for this period. Ensure some nominations and deliveries have been made.")
    else:
