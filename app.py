...
if 'nomination_log' not in st.session_state:
    st.session_state.nomination_log = pd.DataFrame(columns=[
        'timestamp', 'customer', 'type', 'volume_mmbtu', 'status'
    ])
...
if 'delivery_log' not in st.session_state:
    ...
    st.session_state.delivery_log = pd.DataFrame(columns=[
        'date', 'customer_id', 'delivered_mmbtu'
    ])
...
def calculate_settlement(start_date, end_date, customers_data=None):
    ...
    # --- Carbon Emission Factor (Placeholder) ---
    # This factor represents tons of CO2e per MMBTU. In a real scenario, this would be
    # more complex, considering fuel type, transport efficiency, distance, etc.
    carbon_emission_factor_per_mmbtu = 0.05 # Example: 0.05 tons CO2e per MMBTU
    ...
    for _, row in merged.iterrows():
        ...
        carbon_emissions = delivered * carbon_emission_factor_per_mmbtu
        ...
        results.append({
            ...
            'carbon_emissions': round(carbon_emissions, 2) # Added carbon emissions
        })
...
def forecast_demand(customer_id, days=7):
    ...
    forecast_volumes = [max(100, daily_avg + random.uniform(-100, 100)) for _ in range(days)]
...
if module == "📝 Nomination Engine":
    ...
    with col1:
        ...
        with st.form("nomination_form"):
            ...
            volume = st.number_input("Volume (MMBTU)", min_value=0.0, max_value=None, value=1000.0)
            ...
            if submit:
                ...
                if valid:
                    st.success(f"✅ Nomination Confirmed! {volume} MMBTU scheduled for {cust}.")
                    new_del = pd.DataFrame([
                        {
                        'date': datetime.now().strftime('%Y-%m-%d'),
                        'customer_id': cust_id,
                        'delivered_mmbtu': volume * 0.95
                        }
                    ])
...
# ----------------------------
    ...
    if filtered_settlement.empty:
        ...
        k5.metric("🌎 Total CO2e Emissions", f"{filtered_settlement['carbon_emissions'].sum():,.2f} tons")
        ...
        st.subheader("🌳 Carbon Emissions by Segment")
        seg_emissions = filtered_settlement.groupby('segment')['carbon_emissions'].sum().reset_index()
        fig_emissions = px.bar(seg_emissions, x='segment', y='carbon_emissions',
                               title="Total Carbon Emissions by Segment",
                               labels={'carbon_emissions': 'CO2e Emissions (tons)'})
        ...
        display_cols = ['name', 'segment', 'nominated_mmbtu', 'delivered_mmbtu', 'shortfall_mmbtu',
                        'penalty_amount', 'base_revenue', 'total_revenue', 'gross_margin', 'margin_pct', 'price_per_mmbtu', 'carbon_emissions']
        ...
        display_df.columns = ['Customer', 'Segment', 'Nominated (MMBTU)', 'Delivered (MMBTU)', 'Shortfall (MMBTU)',
                              'Penalty', 'Base Rev', 'Total Charge', 'Gross Margin', 'Margin %', 'Price ($/MMBTU)', 'CO2e Emissions (tons)']
...
# ----------------------------
    ...
    selected_customer_name = st.selectbox("Select Customer for Forecasting", options=customers_df['name'])
    selected_customer_id = customers_df[customers_df['name'] == selected_customer_name].iloc[0]['customer_id']

    st.subheader(f"Demand Forecast for {selected_customer_name}")
    forecast_days = st.slider("Forecast Days", min_value=1, max_value=30, value=7)
    forecast_df = forecast_demand(selected_customer_id, days=forecast_days)

    fig_forecast = px.line(forecast_df, x='date', y='forecast_mmbtu',
                            title=f"7-Day Forecasted Demand for {selected_customer_name}",
                            labels={'forecast_mmbtu': 'Forecasted Volume (MMBTU)'})
    st.plotly_chart(fig_forecast, use_container_width=True)

    st.subheader("Anomaly Detection (Last 7 Nominations)")
    # Simulate anomaly detection: Check if latest nomination deviates significantly from forecast
    customer_nominations = st.session_state.nomination_log[
        st.session_state.nomination_log['customer'] == selected_customer_name
    ...
    if not customer_nominations.empty:
        ...
        # Assuming the forecast is for 'today' or the nearest future date
        today_forecast = forecast_df[forecast_df['date'].dt.date == datetime.now().date()]['forecast_mmbtu'].mean()

        if pd.isna(today_forecast):
            st.info("No forecast available for today to compare against.")
        else:
            deviation_pct = ((latest_volume - today_forecast) / today_forecast) * 100
            if abs(deviation_pct) > 20: # Example threshold for anomaly
                st.warning(f"🚨 Anomaly Detected: Latest nomination of {latest_volume:,.0f} MMBTU for {selected_customer_name} deviates by {deviation_pct:+.1f}% from forecasted {today_forecast:,.0f} MMBTU. Investigate!")
            else:
                st.info(f"No significant anomalies detected for {selected_customer_name}. Latest nomination ({latest_volume:,.0f} MMBTU) is in line with forecast ({today_forecast:,.0f} MMBTU).")
    else:
        st.info("No nominations submitted yet for this customer to detect anomalies.")
